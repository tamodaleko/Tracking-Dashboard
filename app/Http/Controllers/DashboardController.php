...
class DashboardController extends Controller
{
    public function index(Request $request): View
    {
        ...
        foreach ($company->campaigns as $campaign) {
            ...
            $data[$campaign->id]['sendCost'] = $orderQuery->clone()->where('order_items.product_id', $campaign->product_id)->count('orders.id') * 102;
            $data[$campaign->id]['shippingCost'] = $orderQuery->clone()->where('order_items.product_id', $campaign->product_id)->where('orders.total', '>', 2000)->count('orders.id') * 280;
